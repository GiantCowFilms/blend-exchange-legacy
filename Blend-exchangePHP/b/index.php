    ...
    $fileExists = $blend->load(['fileName', 'fileGoogleId', 'flags', 'views', 'downloads', 'password', 'uploaderIp', 'questionLink', 'fileSize','deleted','adminComment']);
    ...
    $db->prepare("INSERT INTO `accesses` SET `ref`=:ref, `type`='view', `ip`='".$ipAdress."', `fileId`=:fileId, `date`=NOW()")->execute(array('fileId' => $blend->id,'ref' => $referingAdress));
